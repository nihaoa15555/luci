...
function index()
	...
	entry({"admin", "system", "index"}, cbi("admin/system", {autoapply=true}), _("General"), 1)
	entry({"admin", "system", "passwd"}, form("admin/passwd"), _("Admin Password"), 10)
	entry({"admin", "system", "backup"}, call("action_backup"), _("Backup / Restore"), 80)
	entry({"admin", "system", "upgrade"}, call("action_upgrade"), _("Flash Firmware"), 90)
	entry({"admin", "system", "reboot"}, call("action_reboot"), _("Reboot"), 100)
end
